...
class SelectFunctionsTesting extends IOSuite {
  ...
  it should "retrieve the writetime of a field from Cassandra" in {
    ...
    val chain = for {
      _ <- database.recipes.store(record)
  ...
  it should "retrieve the dateOf of a field from Cassandra" in {
    ...
    val chain = for {
      _ <- database.timeuuidTable.store(record)
  ...
  it should "retrieve the TTL of a field from Cassandra" in {
    ...
    val chain = for {
      _ <- database.timeuuidTable.store(record).ttl(timeToLive)
  ...
  it should "retrieve the sum of an int field from Cassandra" in {
    ...
    val chain = for {
      _ <- database.primitives.store(record)
